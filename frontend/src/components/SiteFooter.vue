<script setup>
import { useRouter } from 'vue-router'
import { useLanguage } from '@/composables/useLanguage'
import { Settings, Download } from 'lucide-vue-next'

const router = useRouter()
const { t } = useLanguage()

const goToAdminSettings = () => {
    router.push('/admin-settings')
}

const goToExport = () => {
    router.push('/export')
}
</script>

<template>
    <!-- Footer -->
    <footer class="bg-gray-900 text-gray-300 py-12 px-6">
        <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            <div class="flex flex-col gap-4">
                <div class="flex items-center gap-3">
                    <img src="/logo.png" alt="Habits Factory Logo" class="h-10 w-10 object-contain" />
                    <span class="text-xl font-bold text-white tracking-tight">Habits Factory</span>
                </div>
                <p class="text-sm text-gray-400">Building better routines, one day at a time.</p>
            </div>
            <div>
                <h3 class="text-white font-semibold mb-4 uppercase text-xs tracking-wider">Management</h3>
                <ul class="space-y-2 text-sm">
                    <li>
                        <button @click="goToAdminSettings"
                            class="hover:text-indigo-400 transition-colors flex items-center gap-2 pl-0">
                            <span>{{ t('adminSettings') }}</span>
                        </button>
                    </li>
                </ul>
            </div>
            <div>
                <h3 class="text-white font-semibold mb-4 uppercase text-xs tracking-wider">Data</h3>
                <ul class="space-y-2 text-sm">
                    <li>
                        <button @click="goToExport"
                            class="hover:text-indigo-400 transition-colors flex items-center gap-2 pl-0">
                            <span>{{ t('importData') }}</span>
                        </button>
                        <button @click="goToExport"
                            class="hover:text-indigo-400 transition-colors flex items-center gap-2 pl-0">
                            <span>{{ t('exportData') }}</span>
                        </button>
                    </li>
                </ul>
            </div>
            <div>
                <h3 class="text-white font-semibold mb-4 uppercase text-xs tracking-wider">Support</h3>
                <a href="https://github.com/habitsfactory/habitsfactory-app" target="_blank" rel="noopener noreferrer"
                    class="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors font-medium flex items-center gap-2 group">
                    <svg class="w-5 h-5 group-hover:scale-110 transition-transform" fill="currentColor"
                        viewBox="0 0 24 24">
                        <path fill-rule="evenodd"
                            d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z"
                            clip-rule="evenodd" />
                    </svg>
                    <span class="text-sm font-bold">{{ t('viewOnGithub') }}</span>
                </a>
            </div>
        </div>
        <div class="max-w-7xl mx-auto mt-12 pt-8 border-t border-gray-800 text-center text-xs text-gray-500">
            © {{ new Date().getFullYear() }} {{ t('appName') }}. {{ t('allRightsReserved') }}
        </div>
    </footer>
</template>